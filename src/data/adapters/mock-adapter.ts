...
/* localStorage is browser-only. The mock adapter runs client-side; every
   method guards `typeof window` so an accidental server import is inert. */
...
export const mockAdapter: DataAdapter = {
  ...
  async getById<T>(collection: string, id: string): Promise<T | null> {
    ...
    return (items.find((item) => item.id === id) as T | undefined) ?? null;
  },
  ...
  async seed(force = false): Promise<void> {
    ...
    // Re-seed cleanly when forced or version changed.
    await this.clear();
    await seedAll(this);
